...
const computeMatch = (inputString, nameOne, nameTwo) => {
  if (inputString.length > 0 && isNumeric(inputString)) {
    ...
    if (myInputAsArray.length > 1) {
      ...
      secondOutput = "";
      if (!tmpString.length > 0 || !isNumeric(tmpString))
        console.log("Here is where I have a bug");
      computeMatch(tmpString, nameOne, nameTwo);
    }
  } else {
    // console.log("Either given string is numeric or length is zero");
  }
};
...
try {
  csv({ noheader: true })
    ...
    .then((users) => {
      ...
      malesWithNoDuplicates.forEach((maleElement) => {
        femalesWithNoDuplicates.forEach((femaleElement) => {
          proccesMatch(maleElement.field1, femaleElement.field1);
        });
      });
    })
    .catch((error) => {
